@extends('frontend.frontend-page-master')
@php
    $post_img = null;
    $blog_image = get_attachment_image_by_id($blog_post->image,"full",false);
    $post_img = !empty($blog_image) ? $blog_image['img_url'] : '';
@endphp

@section('page-title')
    {{$blog_post->getTranslation('title',$user_select_lang_slug)}}
@endsection

@section('page-meta-data')
    {!! render_site_title($blog_post->getTranslation('title',$user_select_lang_slug)) !!}
    {!!  render_page_meta_data($blog_post) !!}
@endsection
@section('content')

    @php
        $session_user_given_password_get = \Illuminate\Support\Facades\Session::get('user_given_password');
    @endphp



        <div class="blog-details-area padding-top-70 padding-bottom-100">
            <div class="container container-two">
                <div class="row">

                 @if($blog_post->visibility == 'public' )
                   <div class="col-xl-8">
                      <x-msg.success/>
                        <div class="blog-details-wrapper">
                            <div class="blog-details-inner">
                                <div class="single-details">
                                @if(!empty($blog_post->image_gallery))
                                    <div class="global-slick-init slick-space-adjust " data-infinite="true" data-slidesToShow="1"
                                         data-slidesToScroll="1" data-speed="500" data-cssEase="linear" data-arrows="false" data-dots="false"
                                         data-prevArrow='<div class="prev-arrow"><i class="las la-arrow-left"></i></div>'
                                         data-nextArrow='<div class="prev-arrow"><i class="las la-arrow-left"></i></div>'
                                         data-autoplaySpeed="2000"
                                         data-responsive='[{"breakpoint": 768,"settings": { "arrows": false,"centerMode": true,"centerPadding": "40px", "slidesToShow": 1}},{"breakpoint": 480, "settings": { "arrows": false, "centerMode": true, "centerPadding": "0px","slidesToShow": 1} }]'
                                    >
                                        @php
                                            $images = explode("|",$blog_post->image_gallery);
                                            $video_url = $blog_post->video_url;
                                        @endphp

                                        @foreach($images as $img)
                                            <div class="single-gallery-image single-featured">
                                                <div class="img-box">
                                                    {!! render_image_markup_by_attachment_id($img,'','large') !!}
                                                    @if($video_url)
                                                        <a href="{{$video_url}}" class="play-icon magnific-inst mfp-iframe v-02" style="color: {{get_static_option('single_page_blog_video_icon_color')}}">
                                                            <i class="las la-play icon"></i>
                                                        </a>
                                                    @endif
                                                </div>
                                            </div>
                                        @endforeach
                                    </div>
                                @else
                                    <div class="details-thumbs bg-custom-image" {!! render_background_image_markup_by_attachment_id($blog_post->image, '', 'large') !!}>
                                        @php
                                            $video_url = $blog_post->video_url;
                                            $icon_color = get_static_option('single_page_blog_video_icon_color');
                                        @endphp
                                        @if($video_url)
                                            <div class="popup-videos">
                                                <a href="{{$video_url}}" class="play-icon videos-play-global" style="color: {{$icon_color}}">
                                                    <i class="las la-play icon"></i>
                                                </a>
                                            </div>
                                        @endif
                                    </div>
                                @endif
                                <div class="details-contents">
                                    @if($blog_post->created_by == 'user')
                                        @php $user = $blog_post->user; @endphp
                                    @else
                                        @php $user = $blog_post->admin; @endphp
                                    @endif
                                    <div class="popular-stories-tag">

                                        <span class="tags"><a @if(!empty($user->id)) href="{{route('frontend.user.created.blog', ['user'=> $blog_post->created_by, 'id'=>$user->id])}}"@endif> <strong> {{$blog_post->author}}  </strong></a> </span>
                                        <span class="tags">{{ date('d M Y',strtotime($blog_post->created_at)) }} </span>
                                        @foreach($blog_post->category_id as $key => $cat)
                                            <span class="tags">
                                                <a href="{{route('frontend.blog.category',['id'=> $cat->id,'any'=> Str::slug($cat->title)])}}"
                                                   class="item">{{$cat->getTranslation('title',$user_select_lang_slug) ?? __('Uncategorized')}}</a>
                                            </span>
                                        @endforeach
                                    </div>
                                    <h2 class="blog-details-title">  {{$blog_post->title ?? __('No Title Given')}} </h2>
                                    <p class="common-para details-para"> {!! $blog_post->blog_content !!} </p>
                                </div>
                            </div>
                        </div>

                        @php
                            $tags_arr = json_decode($blog_post->tag_id);
                            $all_tags = is_array($tags_arr) ? implode(",", $tags_arr) : "";
                        @endphp

                        <div class="details-tag-area padding-top-50 padding-bottom-50">
                         @if(!is_null($tags_arr) && count($tags_arr) > 0)
                            <div class="row">
                                <div class="col-lg-6 col-md-6">
                                    <div class="tags">
                                        <p class="tag-tiitle"> {{__('Tags')}}: </p>

                                        <ul>
                                            @foreach($tags_arr as $i)
                                                @if(!empty($i))
                                                    <li><a href="{{ route('frontend.blog.tags.page', [ 'any'=> $i ?? 'u']) }}">{{$i}}</a></li>
                                                @endif
                                            @endforeach
                                        </ul>
                                    </div>
                                </div>
                                <div class="col-lg-6 col-md-6">
                                    <div class="social-share">
                                        <p class="share-tiitle"> {{__('Share')}}: </p>
                                         <ul>
                                             {!! single_post_share(route('frontend.blog.single',['id' => $blog_post->id, 'slug' => Str::slug($blog_post->title,'-')]),$blog_post->title,$blog_post->image) !!}
                                        </ul>
                                    </div>
                                </div>
                            </div>
                             @endif
                        </div>

                        <div class="details-comment-area padding-bottom-50" id="comment-area">
                            <div class="section-title">
                                @if($blogCommentCount > 0)
                                    <h4 class="title">
                                        {{sprintf('%s %s ',
                                        $blogCommentCount,
                                           get_static_option( 'blog_single_page_comments_'.get_user_lang().'_text')
                                        )}}

                                    </h4>
                                @endif
                            </div>
                            <div class="comments-inner">

                                    <div class="comments-flex-contents" id="comment_content_div">
                                        {{ csrf_field() }}
                                        <div id="comment_data" data-items="5">
                                            @include('frontend.partials.pages-portion.comment-show-data')
                                        </div>

                                        @if($blogComments->count())
                                            @if($blogComments->count() > 4)
                                                <div class="load_more_div mt-4">
                                                    <button type="button" class="load-more-btn" id="load_more_comment_button">{{__('Load More')}}</button>
                                                </div>
                                            @endif
                                        @endif
                                    </div>

                            </div>
                        </div>

                        @if(!auth()->guard('web')->check())
                            @include('frontend.partials.ajax-user-login-markup',['title' => get_static_option('blog_single_page_login_title_'.$user_select_lang_slug.'_text')])
                        @endif

                        <div class="details-replay-area padding-bottom-30">
                            @if(auth()->guard('web')->check())
                            <div class="section-title">
                                <h3 class="title">{!! get_static_option('blog_single_page_comments_'.get_user_lang().'_title_text') !!}</h3>
                            </div>
                            <div class="replay-inner">
                                <form action="{{route('blog.comment.store')}}" class="comment-form" id="blog-comment-form">
                                    @csrf
                                    <div class="error-message"></div>
                                    <input type="hidden" name="comment_id" />
                                    <input type="hidden" name="blog_id" id="blog_id"
                                           value="{{$blog_post->id}}">
                                    <input type="hidden" name="user_id" id="user_id"
                                           value="{{auth()->guard('web')->user()->id}}">

                                    <input type="hidden" name="commented_by" id="commented_by"
                                           value="{{auth()->guard('web')->user()->name}}">

                                    <div class="replay-inner">
                                        <div class="form-group">
                                           <textarea name="comment_content" id="comment_content" class="form-control" placeholder="Comments"
                                             cols="30" rows="10" required=""></textarea>
                                        </div>

                                        <div class="btn-wrapper">
                                            <button id="submitComment" type="submit" class="btn-default">{!! get_static_option('blog_single_page_comments_button_'.get_user_lang().'_text') !!}</button>
                                        </div>

                                    </div>

                                </form>
                                @endif
                            </div>
                        </div>

                        @if(count($all_related_blog) > 0)
                        <div class="related-post-area padding-top-50">
                            <div class="section-title">
                                <h4 class="title"> {{__('Related Post')}} </h4>
                            </div>
                            <div class="row">

                                @foreach($all_related_blog as $data)
                                <div class="col-lg-6">
                                    <div class="single-news margin-top-30">
                                        @php
                                            $video_url = $data->video_url;
                                            $icon_color = get_static_option('single_page_blog_video_icon_color');
                                        @endphp

                                        <div class="news-thumb">
                                           {!! render_image_markup_by_attachment_id($data->image, '' ,'thumbnail') !!}
                                            @if($video_url)
                                                <div class="popup-videos">
                                                    <a href="{{$video_url}}" class="play-icon videos-play-global videos-play-medium" style="color: {{$icon_color}}">
                                                        <i class="las la-play icon"></i>
                                                    </a>
                                                </div>
                                            @endif
                                            <ul class="news-date-tag">
                                                <li class="tag-list"> {{ date('d M Y', strtotime($data->created_at)) }} </li>

                                                @foreach($data->category_id as $key => $cat)
                                                    <li class="tag-list">
                                                <a href="{{route('frontend.blog.category',['id'=> $cat->id,'any'=> Str::slug($cat->title)])}}"
                                                   class="item">{{$cat->getTranslation('title',$user_select_lang_slug) ?? __('Uncategorized')}}</a>
                                            </li>
                                                @endforeach
                                            </ul>
                                        </div>
                                        <div class="news-contents">
                                            <h3 class="common-title"> <a href="{{route('frontend.blog.single',$data->slug)}}"> {{$data->title ?? ''}} </a> </h3>
                                        </div>
                                    </div>
                                </div>
                                @endforeach
                            </div>
                        </div>
                        @endif
                   </div>
               


                  @elseif($blog_post->visibility == 'logged_user' )

                   @if(auth()->guard('web')->check())
                    <div class="col-xl-8">
                      <x-msg.success/>
                        <div class="blog-details-wrapper">
                            <div class="blog-details-inner">
                                <div class="single-details">
                                @if(!empty($blog_post->image_gallery))
                                    <div class="global-slick-init slick-space-adjust " data-infinite="true" data-slidesToShow="1"
                                         data-slidesToScroll="1" data-speed="500" data-cssEase="linear" data-arrows="false" data-dots="false"
                                         data-prevArrow='<div class="prev-arrow"><i class="las la-arrow-left"></i></div>'
                                         data-nextArrow='<div class="prev-arrow"><i class="las la-arrow-left"></i></div>'
                                         data-autoplaySpeed="2000"
                                         data-responsive='[{"breakpoint": 768,"settings": { "arrows": false,"centerMode": true,"centerPadding": "40px", "slidesToShow": 1}},{"breakpoint": 480, "settings": { "arrows": false, "centerMode": true, "centerPadding": "0px","slidesToShow": 1} }]'
                                    >
                                        @php
                                            $images = explode("|",$blog_post->image_gallery);
                                            $video_url = $blog_post->video_url;
                                        @endphp

                                        @foreach($images as $img)
                                            <div class="single-gallery-image single-featured">
                                                <div class="img-box">
                                                    {!! render_image_markup_by_attachment_id($img,'','large') !!}
                                                    @if($video_url)
                                                        <a href="{{$video_url}}" class="play-icon magnific-inst mfp-iframe v-02" style="color: {{get_static_option('single_page_blog_video_icon_color')}}">
                                                            <i class="las la-play icon"></i>
                                                        </a>
                                                    @endif
                                                </div>
                                            </div>
                                        @endforeach
                                    </div>
                                @else
                                    <div class="details-thumbs bg-custom-image" {!! render_background_image_markup_by_attachment_id($blog_post->image, '', 'large') !!}>
                                        @php
                                            $video_url = $blog_post->video_url;
                                            $icon_color = get_static_option('single_page_blog_video_icon_color');
                                        @endphp
                                        @if($video_url)
                                            <div class="popup-videos">
                                                <a href="{{$video_url}}" class="play-icon videos-play-global" style="color: {{$icon_color}}">
                                                    <i class="las la-play icon"></i>
                                                </a>
                                            </div>
                                        @endif
                                    </div>
                                @endif
                                <div class="details-contents">
                                    @if($blog_post->created_by == 'user')
                                        @php $user = $blog_post->user; @endphp
                                    @else
                                        @php $user = $blog_post->admin; @endphp
                                    @endif
                                    <div class="popular-stories-tag">

                                        <span class="tags"><a @if(!empty($user->id)) href="{{route('frontend.user.created.blog', ['user'=> $blog_post->created_by, 'id'=>$user->id])}}"@endif> <strong> {{$blog_post->author}}  </strong></a> </span>
                                        <span class="tags">{{ date('d M Y',strtotime($blog_post->created_at)) }} </span>
                                        @foreach($blog_post->category_id as $key => $cat)
                                            <span class="tags">
                                                <a href="{{route('frontend.blog.category',['id'=> $cat->id,'any'=> Str::slug($cat->title)])}}"
                                                   class="item">{{$cat->getTranslation('title',$user_select_lang_slug) ?? __('Uncategorized')}}</a>
                                            </span>
                                        @endforeach
                                    </div>
                                    <h2 class="blog-details-title">  {{$blog_post->title ?? __('No Title Given')}} </h2>
                                    <p class="common-para details-para"> {!! $blog_post->blog_content !!} </p>
                                </div>
                            </div>
                        </div>

                        @php
                            $tags_arr = json_decode($blog_post->tag_id);
                            $all_tags = is_array($tags_arr) ? implode(",", $tags_arr) : "";
                        @endphp

                        <div class="details-tag-area padding-top-50 padding-bottom-50">
                         @if(!is_null($tags_arr) && count($tags_arr) > 0)
                            <div class="row">
                                <div class="col-lg-6 col-md-6">
                                    <div class="tags">
                                        <p class="tag-tiitle"> {{__('Tags')}}: </p>

                                        <ul>
                                            @foreach($tags_arr as $i)
                                                @if(!empty($i))
                                                    <li><a href="{{ route('frontend.blog.tags.page', [ 'any'=> $i ?? 'u']) }}">{{$i}}</a></li>
                                                @endif
                                            @endforeach
                                        </ul>
                                    </div>
                                </div>
                                <div class="col-lg-6 col-md-6">
                                    <div class="social-share">
                                        <p class="share-tiitle"> {{__('Share')}}: </p>
                                         <ul>
                                             {!! single_post_share(route('frontend.blog.single',['id' => $blog_post->id, 'slug' => Str::slug($blog_post->title,'-')]),$blog_post->title,$blog_post->image) !!}
                                        </ul>
                                    </div>
                                </div>
                            </div>
                             @endif
                        </div>

                        <div class="details-comment-area padding-bottom-50" id="comment-area">
                            <div class="section-title">
                                @if($blogCommentCount > 0)
                                    <h4 class="title">
                                        {{sprintf('%s %s ',
                                        $blogCommentCount,
                                           get_static_option( 'blog_single_page_comments_'.get_user_lang().'_text')
                                        )}}

                                    </h4>
                                @endif
                            </div>
                            <div class="comments-inner">

                                    <div class="comments-flex-contents" id="comment_content_div">
                                        {{ csrf_field() }}
                                        <div id="comment_data" data-items="5">
                                            @include('frontend.partials.pages-portion.comment-show-data')
                                        </div>

                                        @if($blogComments->count())
                                            @if($blogComments->count() > 4)
                                                <div class="load_more_div mt-4">
                                                    <button type="button" class="load-more-btn" id="load_more_comment_button">{{__('Load More')}}</button>
                                                </div>
                                            @endif
                                        @endif
                                    </div>

                            </div>
                        </div>

                        @if(!auth()->guard('web')->check())
                            @include('frontend.partials.ajax-user-login-markup',['title' => get_static_option('blog_single_page_login_title_'.$user_select_lang_slug.'_text')])
                        @endif

                        <div class="details-replay-area padding-bottom-30">
                            @if(auth()->guard('web')->check())
                            <div class="section-title">
                                <h3 class="title">{!! get_static_option('blog_single_page_comments_'.get_user_lang().'_title_text') !!}</h3>
                            </div>
                            <div class="replay-inner">
                                <form action="{{route('blog.comment.store')}}" class="comment-form" id="blog-comment-form">
                                    @csrf
                                    <div class="error-message"></div>
                                    <input type="hidden" name="comment_id" />
                                    <input type="hidden" name="blog_id" id="blog_id"
                                           value="{{$blog_post->id}}">
                                    <input type="hidden" name="user_id" id="user_id"
                                           value="{{auth()->guard('web')->user()->id}}">

                                    <input type="hidden" name="commented_by" id="commented_by"
                                           value="{{auth()->guard('web')->user()->name}}">

                                    <div class="replay-inner">
                                        <div class="form-group">
                                           <textarea name="comment_content" id="comment_content" class="form-control" placeholder="Comments"
                                             cols="30" rows="10" required=""></textarea>
                                        </div>

                                        <div class="btn-wrapper">
                                            <button id="submitComment" type="submit" class="btn-default">{!! get_static_option('blog_single_page_comments_button_'.get_user_lang().'_text') !!}</button>
                                        </div>

                                    </div>

                                </form>
                                @endif
                            </div>
                        </div>

                        @if(count($all_related_blog) > 0)
                        <div class="related-post-area padding-top-50">
                            <div class="section-title">
                                <h4 class="title"> {{__('Related Post')}} </h4>
                            </div>
                            <div class="row">

                                @foreach($all_related_blog as $data)
                                <div class="col-lg-6">
                                    <div class="single-news margin-top-30">
                                        @php
                                            $video_url = $data->video_url;
                                            $icon_color = get_static_option('single_page_blog_video_icon_color');
                                        @endphp

                                        <div class="news-thumb">
                                           {!! render_image_markup_by_attachment_id($data->image, '' ,'thumbnail') !!}
                                            @if($video_url)
                                                <div class="popup-videos">
                                                    <a href="{{$video_url}}" class="play-icon videos-play-global videos-play-medium" style="color: {{$icon_color}}">
                                                        <i class="las la-play icon"></i>
                                                    </a>
                                                </div>
                                            @endif
                                            <ul class="news-date-tag">
                                                <li class="tag-list"> {{ date('d M Y', strtotime($data->created_at)) }} </li>

                                                @foreach($data->category_id as $key => $cat)
                                                    <li class="tag-list">
                                                <a href="{{route('frontend.blog.category',['id'=> $cat->id,'any'=> Str::slug($cat->title)])}}"
                                                   class="item">{{$cat->getTranslation('title',$user_select_lang_slug) ?? __('Uncategorized')}}</a>
                                            </li>
                                                @endforeach
                                            </ul>
                                        </div>
                                        <div class="news-contents">
                                            <h3 class="common-title"> <a href="{{route('frontend.blog.single',$data->slug)}}"> {{$data->title ?? ''}} </a> </h3>
                                        </div>
                                    </div>
                                </div>
                                @endforeach
                            </div>
                        </div>
                        @endif
                    </div>
                 </div>
                

                     @else
                      <div class="col-xl-8">
                            <div class="alert alert-warning">
                                <h3>{{__('Login to see the blog details')}}</h3>
                            </div>
                       </div>
                    @endif

                 

                  @elseif($blog_post->visibility == 'password' && $blog_post->password != $session_user_given_password_get )

                  <div class="col-xl-8">
                   <x-msg.error/>
                    <x-msg.success/>
                        <form action="{{route('frontend.user.blog.password')}}" method="get">
                            <label for="">{{__('Please Provide Blog Password to see this blog details (If you have no password then contact to the admin..!)')}}</label>
                            <input class="form-control" type="password" name="user_blog_password" style="height: 50px">
                            <input type="hidden" name="original_password" value="{{$blog_post->password}}">
                            <input type="hidden" name="password_form_id" value="{{$blog_post->id}}">

                            <button class="btn btn-primary btn-md mt-3" type="submit">{{__('Submit')}}</button>
                        </form>
                    </div>


                   @elseif($blog_post->visibility == 'password' && (!is_null($blog_post->password) == $session_user_given_password_get))
              
                   <div class="col-xl-8">
                      <x-msg.success/>
                        <div class="blog-details-wrapper">
                            <div class="blog-details-inner">
                                <div class="single-details">
                                @if(!empty($blog_post->image_gallery))
                                    <div class="global-slick-init slick-space-adjust " data-infinite="true" data-slidesToShow="1"
                                         data-slidesToScroll="1" data-speed="500" data-cssEase="linear" data-arrows="false" data-dots="false"
                                         data-prevArrow='<div class="prev-arrow"><i class="las la-arrow-left"></i></div>'
                                         data-nextArrow='<div class="prev-arrow"><i class="las la-arrow-left"></i></div>'
                                         data-autoplaySpeed="2000"
                                         data-responsive='[{"breakpoint": 768,"settings": { "arrows": false,"centerMode": true,"centerPadding": "40px", "slidesToShow": 1}},{"breakpoint": 480, "settings": { "arrows": false, "centerMode": true, "centerPadding": "0px","slidesToShow": 1} }]'
                                    >
                                        @php
                                            $images = explode("|",$blog_post->image_gallery);
                                            $video_url = $blog_post->video_url;
                                        @endphp

                                        @foreach($images as $img)
                                            <div class="single-gallery-image single-featured">
                                                <div class="img-box">
                                                    {!! render_image_markup_by_attachment_id($img,'','large') !!}
                                                    @if($video_url)
                                                        <a href="{{$video_url}}" class="play-icon magnific-inst mfp-iframe v-02" style="color: {{get_static_option('single_page_blog_video_icon_color')}}">
                                                            <i class="las la-play icon"></i>
                                                        </a>
                                                    @endif
                                                </div>
                                            </div>
                                        @endforeach
                                    </div>
                                @else
                                    <div class="details-thumbs bg-custom-image" {!! render_background_image_markup_by_attachment_id($blog_post->image, '', 'large') !!}>
                                        @php
                                            $video_url = $blog_post->video_url;
                                            $icon_color = get_static_option('single_page_blog_video_icon_color');
                                        @endphp
                                        @if($video_url)
                                            <div class="popup-videos">
                                                <a href="{{$video_url}}" class="play-icon videos-play-global" style="color: {{$icon_color}}">
                                                    <i class="las la-play icon"></i>
                                                </a>
                                            </div>
                                        @endif
                                    </div>
                                @endif
                                <div class="details-contents">
                                    @if($blog_post->created_by == 'user')
                                        @php $user = $blog_post->user; @endphp
                                    @else
                                        @php $user = $blog_post->admin; @endphp
                                    @endif
                                    <div class="popular-stories-tag">

                                        <span class="tags"><a @if(!empty($user->id)) href="{{route('frontend.user.created.blog', ['user'=> $blog_post->created_by, 'id'=>$user->id])}}"@endif> <strong> {{$blog_post->author}}  </strong></a> </span>
                                        <span class="tags">{{ date('d M Y',strtotime($blog_post->created_at)) }} </span>
                                        @foreach($blog_post->category_id as $key => $cat)
                                            <span class="tags">
                                                <a href="{{route('frontend.blog.category',['id'=> $cat->id,'any'=> Str::slug($cat->title)])}}"
                                                   class="item">{{$cat->getTranslation('title',$user_select_lang_slug) ?? __('Uncategorized')}}</a>
                                            </span>
                                        @endforeach
                                    </div>
                                    <h2 class="blog-details-title">  {{$blog_post->title ?? __('No Title Given')}} </h2>
                                    <p class="common-para details-para"> {!! $blog_post->blog_content !!} </p>
                                </div>
                            </div>
                        </div>

                        @php
                            $tags_arr = json_decode($blog_post->tag_id);
                            $all_tags = is_array($tags_arr) ? implode(",", $tags_arr) : "";
                        @endphp

                        <div class="details-tag-area padding-top-50 padding-bottom-50">
                         @if(!is_null($tags_arr) && count($tags_arr) > 0)
                            <div class="row">
                                <div class="col-lg-6 col-md-6">
                                    <div class="tags">
                                        <p class="tag-tiitle"> {{__('Tags')}}: </p>

                                        <ul>
                                            @foreach($tags_arr as $i)
                                                @if(!empty($i))
                                                    <li><a href="{{ route('frontend.blog.tags.page', [ 'any'=> $i ?? 'u']) }}">{{$i}}</a></li>
                                                @endif
                                            @endforeach
                                        </ul>
                                    </div>
                                </div>
                                <div class="col-lg-6 col-md-6">
                                    <div class="social-share">
                                        <p class="share-tiitle"> {{__('Share')}}: </p>
                                         <ul>
                                             {!! single_post_share(route('frontend.blog.single',['id' => $blog_post->id, 'slug' => Str::slug($blog_post->title,'-')]),$blog_post->title,$blog_post->image) !!}
                                        </ul>
                                    </div>
                                </div>
                            </div>
                             @endif
                        </div>

                        <div class="details-comment-area padding-bottom-50" id="comment-area">
                            <div class="section-title">
                                @if($blogCommentCount > 0)
                                    <h4 class="title">
                                        {{sprintf('%s %s ',
                                        $blogCommentCount,
                                           get_static_option( 'blog_single_page_comments_'.get_user_lang().'_text')
                                        )}}

                                    </h4>
                                @endif
                            </div>
                            <div class="comments-inner">

                                    <div class="comments-flex-contents" id="comment_content_div">
                                        {{ csrf_field() }}
                                        <div id="comment_data" data-items="5">
                                            @include('frontend.partials.pages-portion.comment-show-data')
                                        </div>

                                        @if($blogComments->count())
                                            @if($blogComments->count() > 4)
                                                <div class="load_more_div mt-4">
                                                    <button type="button" class="load-more-btn" id="load_more_comment_button">{{__('Load More')}}</button>
                                                </div>
                                            @endif
                                        @endif
                                    </div>

                            </div>
                        </div>

                        @if(!auth()->guard('web')->check())
                            @include('frontend.partials.ajax-user-login-markup',['title' => get_static_option('blog_single_page_login_title_'.$user_select_lang_slug.'_text')])
                        @endif

                        <div class="details-replay-area padding-bottom-30">
                            @if(auth()->guard('web')->check())
                            <div class="section-title">
                                <h3 class="title">{!! get_static_option('blog_single_page_comments_'.get_user_lang().'_title_text') !!}</h3>
                            </div>
                            <div class="replay-inner">
                                <form action="{{route('blog.comment.store')}}" class="comment-form" id="blog-comment-form">
                                    @csrf
                                    <div class="error-message"></div>
                                    <input type="hidden" name="comment_id" />
                                    <input type="hidden" name="blog_id" id="blog_id"
                                           value="{{$blog_post->id}}">
                                    <input type="hidden" name="user_id" id="user_id"
                                           value="{{auth()->guard('web')->user()->id}}">

                                    <input type="hidden" name="commented_by" id="commented_by"
                                           value="{{auth()->guard('web')->user()->name}}">

                                    <div class="replay-inner">
                                        <div class="form-group">
                                           <textarea name="comment_content" id="comment_content" class="form-control" placeholder="Comments"
                                             cols="30" rows="10" required=""></textarea>
                                        </div>

                                        <div class="btn-wrapper">
                                            <button id="submitComment" type="submit" class="btn-default">{!! get_static_option('blog_single_page_comments_button_'.get_user_lang().'_text') !!}</button>
                                        </div>

                                    </div>

                                </form>
                                @endif
                            </div>
                        </div>

                        @if(count($all_related_blog) > 0)
                        <div class="related-post-area padding-top-50">
                            <div class="section-title">
                                <h4 class="title"> {{__('Related Post')}} </h4>
                            </div>
                            <div class="row">

                                @foreach($all_related_blog as $data)
                                <div class="col-lg-6">
                                    <div class="single-news margin-top-30">
                                        @php
                                            $video_url = $data->video_url;
                                            $icon_color = get_static_option('single_page_blog_video_icon_color');
                                        @endphp

                                        <div class="news-thumb">
                                           {!! render_image_markup_by_attachment_id($data->image, '' ,'thumbnail') !!}
                                            @if($video_url)
                                                <div class="popup-videos">
                                                    <a href="{{$video_url}}" class="play-icon videos-play-global videos-play-medium" style="color: {{$icon_color}}">
                                                        <i class="las la-play icon"></i>
                                                    </a>
                                                </div>
                                            @endif
                                            <ul class="news-date-tag">
                                                <li class="tag-list"> {{ date('d M Y', strtotime($data->created_at)) }} </li>

                                                @foreach($data->category_id as $key => $cat)
                                                    <li class="tag-list">
                                                <a href="{{route('frontend.blog.category',['id'=> $cat->id,'any'=> Str::slug($cat->title)])}}"
                                                   class="item">{{$cat->getTranslation('title',$user_select_lang_slug) ?? __('Uncategorized')}}</a>
                                            </li>
                                                @endforeach
                                            </ul>
                                        </div>
                                        <div class="news-contents">
                                            <h3 class="common-title"> <a href="{{route('frontend.blog.single',$data->slug)}}"> {{$data->title ?? ''}} </a> </h3>
                                        </div>
                                    </div>
                                </div>
                                @endforeach
                            </div>
                        </div>
                        @endif
                   </div>
                 

                @else
  
                <div class="col-xl-8">
                   <x-msg.error/>
                    <x-msg.success/>
                        <form action="{{route('frontend.user.blog.password')}}" method="get">
                            <label for="">{{__('Please Provide Blog Password to see this blog details (If you have no password then contact to the admin..!)')}}</label>
                            <input class="form-control" type="password" name="user_blog_password" style="height: 50px">
                            <input type="hidden" name="original_password" value="{{$blog_post->password}}">
                            <input type="hidden" name="password_form_id" value="{{$blog_post->id}}">

                            <button class="btn btn-primary btn-md mt-3" type="submit">{{__('Submit')}}</button>
                        </form>
                    </div>
                @endif

                <div class="col-xl-4 col-lg-8 margin-top-30">
                    <div class="widget-area-wrapper">
                        {!! render_frontend_sidebar('sidebar_01',['column' => false]) !!}
                    </div>
                </div>

            </div>
        </div>
    </div>
    </div>


@endsection

@section('scripts')
    <script>
        (function($){
            "use strict";

            $(document).ready(function(){
                //Blog Comment Insert
                $(document).on('click', '#submitComment', function (e) {
                    e.preventDefault();
                    var erContainer = $(".error-message");
                    var el = $(this);
                    var form = $('#blog-comment-form');
                    var user_id = $('#user_id').val();
                    var blog_id = $('#blog_id').val();
                    var commented_by = $('#commented_by').val();
                    var comment_content = $('#comment_content').val();
                    let comment_id = $('#blog-comment-form input[name=comment_id]').val();
                    el.text('{{__('Submitting')}}...');

                    $.ajax({
                        url: form.attr('action'),
                        method: 'POST',
                        data: {
                            _token: "{{csrf_token()}}",
                            user_id: user_id,
                            blog_id: blog_id,
                            commented_by: commented_by,
                            comment_id: comment_id,
                            comment_content: comment_content,
                        },
                        success: function (data){
                              location.reload();
                            $('#comment_content').val('');
                            // erContainer.html('<div class="alert alert- '+data.msg+'"></div>');
                            {{--load_comment_data('{{$blog_post->id}}');--}}
                        },
                        error: function (data) {
                            var errors = data.responseJSON;
                            erContainer.html('<div class="alert alert-danger"></div>');
                            $.each(errors.errors, function (index, value) {
                                erContainer.find('.alert.alert-danger').append('<p>' + value + '</p>');
                            });
                            el.text('{{__('Comment')}}');
                        },

                    });
                });

                //Blog Replay
                $(document).on('click', '.btn-replay', function (e) {
                    e.preventDefault();
                    var el = $(this).hide();
                    let comment_id = $(this).data('comment_id');
                    let parent_name = $(this).parent().parent().find('.author-title').data('parent_name');

                    $('#blog-comment-form input[name=comment_id]').val(comment_id);
                    $('#comment_content').attr('placeholder','Replying to '+ parent_name + '..');

                });

                function load_comment_data(id) {
                    var commentData = $('#comment_data');
                    var items = commentData.attr('data-items');
                    $.ajax({
                        url: "{{ route('frontend.load.blog.comment.data') }}",
                        method: "POST",
                        data: {id: id, _token: "{{csrf_token()}}", items: items},
                        success: function (data) {

                            commentData.attr('data-items',parseInt(items) + 5);

                            $('#comment_data').append(data.markup);
                            $('#load_more_comment_button').text('{{__('Load More')}}');


                            if (data.blogComments.length === 0) {
                                $('#load_more_comment_button').text('{{__('No Comment Found')}}');
                            }

                        }
                    })
                }

                $(document).on('click', '#load_more_comment_button', function () {
                    $(this).text('{{__('Loading...')}}');
                    load_comment_data('{{$blog_post->id}}');

                });

            });
        })(jQuery);
    </script>
@endsection
